...
public class TransactionDetail : TransactionDetailResource
{
    public static TransactionDetail Retrieve(RetrieveTransactionDetailRequest request, Options options)
    {
        ...
        if (string.IsNullOrEmpty(request.PaymentId))
            url =
                $"{options.BaseUrl}/v2/reporting/payment/details?paymentConversationId={request.PaymentConversationId}";
        else
            url = $"{options.BaseUrl}/v2/reporting/payment/details?paymentId={request.PaymentId}";
